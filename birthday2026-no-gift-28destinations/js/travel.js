...
document.getElementById("countdownButton").addEventListener("click", () => {
  const winnerName = document.getElementById("destinationResult").textContent.trim();
  const targetUrl = winnerName && winnerName !== "" ? `countdown.html?destination=${encodeURIComponent(winnerName)}` : "countdown.html";
